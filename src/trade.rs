...
async fn confirm_trade_interactive(wallet: &Wallet,
    ...
    let slp_tx_route = format!("https://api.calory.money/parse_slp_tx?txid={}&vout={}", utxo.txid, utxo.vout);
...
pub async fn accept_trades_interactive(wallet: &Wallet,  token_symbol: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = reqwest::get(
        "https://api.calory.money/trades").await?;
        ...
        .then(|chunk| {
            ...
            async move {
                let futures = chunk.into_iter().map(|txid| {
                    let url = format!("https://api.calory.money/validate_slp/{}", txid);
                    client.get(&url).send()
                });
    ...
    for chunk in valid_txs.iter().collect::<Vec<_>>().chunks(20) {
        ...
        let url = format!("https://api.calory.money/tx_details?{}", chunk.iter()
